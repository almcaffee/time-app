...
import { HttpEvent, HttpHandler, HttpRequest, HttpHeaders, HttpInterceptor, HttpResponse, HttpProgressEvent } from '@angular/common/http';
...
export class HttpRequestInterceptor implements HttpInterceptor {
  ...
  intercept(req: HttpRequest<any>, next: any): Observable<any> {
    ...
    } else if(req.method === 'POST'|| req.method === 'PUT' && (req.url.indexOf('img') > 0 || req.url.indexOf('file') > 0)) {
      const headers = this.getBlobHeader();
